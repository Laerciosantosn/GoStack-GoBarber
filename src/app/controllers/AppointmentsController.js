...
class AppointmentsController {
  ...
  async store(req, res) {
    ...
    //  o req.userId nao pode ser o mesmo que o provider_id
    // check if provider_id is a provider
    if (req.userId === provider_id) {
      return res
        .status(400)
        .json({ error: 'Não pode agendar para voce mesmo' });
    }
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res
        .status(401)
        .json({ error: 'You can only cancel apponitments 2 hours in advance' });
    }
